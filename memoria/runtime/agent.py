...
class AgentRuntime:
    ...
    async def run(self, session_id: str, user_text: str) -> tuple[dict, list[dict], dict]:
        ...
        try:
            ...
            tracer.finish("failed", len(context.tool_chain), context.memories, context.tool_chain, f"{type(exc).__name__}: {exc}")
            raise
