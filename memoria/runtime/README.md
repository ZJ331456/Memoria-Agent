...
9. 执行 `after_reasoning`，保存助手消息，异步语义提取记忆并去重。
...
新的推理策略应依赖 `Store`、`LLMClient`、`MemoryEngine`、`ToolRegistry` 接口，不要把 SQL 或 HTTP 写进 runtime。未来可新增流式 delta、取消令牌、上下文预算和并行只读工具，但应保持 `run(session_id, user_text)` 作为稳定入口。
