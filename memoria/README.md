...
根级文件职责：`config.py` 管理配置，`store.py` 管理 SQLite，`vector_index.py` 提供可选 sqlite-vec KNN，`llm.py` 适配 OpenAI-compatible API，`security.py` 实现可选请求安全边界，`service.py` 保持应用服务入口；`api/` 提供 HTTP 与静态站点。

`api/app.py` 集中定义公开 API 的严格请求/响应契约、异常协议、中间件、路由和 SPA 托管，`api/__init__.py` 保持 `from memoria.api import create_app` 兼容。代码级说明见 [`api/README.md`](api/README.md)，完整接口说明位于 [`../docs/API接口文档.md`](../docs/API接口文档.md)，运行时 OpenAPI 位于 `/docs` 与 `/openapi.json`。
